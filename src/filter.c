...
/*   Updated: 2017/07/11 15:45:16 by pde-maul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			filter_sepia(t_env *e, int x, int y)
{
	...
	while (y <= e->mlx->h / e->anti_a)
	{
		...
		c2[0] = (0.393 * rgb[0] + 0.769 * rgb[1] + 0.189 * rgb[2]) * sepia +
		(1 - sepia) * rgb[0];
		c2[1] = (0.349 * rgb[0] + 0.686 * rgb[1] + 0.168 * rgb[2]) * sepia +
		(1 - sepia) * rgb[1];
		c2[2] = (0.272 * rgb[0] + 0.534 * rgb[1] + 0.131 * rgb[2]) * sepia +
		(1 - sepia) * rgb[2];
