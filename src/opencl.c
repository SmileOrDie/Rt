...
/*   Updated: 2017/06/28 19:24:11 by pde-maul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_init_opencl(t_env *e, t_opencl *cl)
{
	...
	err = ft_init_opencl2(cl, namefile);
	i = -1;
	while (++i < 5)
		source[i] = ft_create_source(namefile[i]);
	cl->kernel_program = clCreateProgramWithSource(cl->context, 5, \
		(const char **)source, 0, &err);
	err ? ft_error(KERNEL, ft_strjoin("clCreateProgramWithSource -> \
	cl->kernel_program -> ", ft_itoa(err))) : 0;
...
void		ft_launch_calc(t_env *e, t_opencl *cl)
{
	...
	err ? ft_error(KERNEL, ft_strjoin("clCreateKernel -> cl->kernel -> ", \
	ft_itoa(err))) : 0;
	cl->obj = clCreateBuffer(cl->context, CL_MEM_READ_WRITE | \
		CL_MEM_COPY_HOST_PTR, sizeof(t_obj) * e->nb_obj, e->l_obj, &err);
	err ? ft_error(KERNEL, ft_strjoin("clCreateBuffer -> e->l_obj -> ", \
	ft_itoa(err))) : 0;
	cl->light = NULL;
	if (e->nb_light > 0)
	{
	...
	err ? ft_error(KERNEL, ft_strjoin("clCreateBuffer -> \
	e->cl_e->color_lst -> ", ft_itoa(err))) : 0;
	cl->texture = NULL;
	if (e->nb_tex > 0)
	{
		...
		err ? ft_error(KERNEL, ft_strjoin("clCreateBuffer -> \
		e->cl_e->texture -> ", ft_itoa(err))) : 0;
	}
	(err = clSetKernelArg(cl->kernel, 5, sizeof(cl_mem), (void *)&cl->texture))\
	? ft_error(KERNEL, ft_strjoin("clSetKernelArg 0 -> ", ft_itoa(err))) : 0;
	(err = clSetKernelArg(cl->kernel, 4, sizeof(cl_mem), (void *)&cl->lst)) ? \
	ft_error(KERNEL, ft_strjoin("clSetKernelArg 0 -> ", ft_itoa(err))) : 0;
	(err = clSetKernelArg(cl->kernel, 1, sizeof(cl_mem), (void *)&cl->obj)) ? \
	ft_error(KERNEL, ft_strjoin("clSetKernelArg 1 -> ", ft_itoa(err))) : 0;
	(err = clSetKernelArg(cl->kernel, 2, sizeof(cl_mem), (void *)&cl->light)) ?\
	ft_error(KERNEL, ft_strjoin("clSetKernelArg 2 -> ", ft_itoa(err))) : 0;
	(err = clSetKernelArg(cl->kernel, 3, sizeof(cl_mem), (void *)&cl->env)) ?\
	ft_error(KERNEL, ft_strjoin("clSetKernelArg 3 -> ", ft_itoa(err))) : 0;
	(err = clSetKernelArg(cl->kernel, 0, sizeof(cl_mem), \
	(void *)&cl->color_lst)) ? ft_error(KERNEL, \
		ft_strjoin("clSetKernelArg 4 -> ", ft_itoa(err))) : 0;
	global[0] = *(e->nb_obj_pix[0]);
	(err = clEnqueueNDRangeKernel(cl->command_queue, cl->kernel, 1, 0, global, \
		0, 0, 0, 0)) ? ft_error(KERNEL, ft_strjoin("clEnqueueNDRangeKernel", \
		ft_itoa(err))) : 0;
